...
def create_model() -> Pipeline:
    ...
    MODEL = MultilayerPerceptronClassifier(
        layers=[45, 44, 43, 44, 2], solver="l-bfgs", seed=123, labelCol="TEP"
    )

    ASSEMBLER_NUM = VectorAssembler(
        inputCols=NUMS, outputCol="num_features", handleInvalid="skip"
    )
    ...
    ASSEMBLER_BOOL = VectorAssembler(
        inputCols=BOOL, outputCol="bool_features", handleInvalid="skip"
    )

    FINAL_ASSEMBLER = VectorAssembler(
        inputCols=["scaled_num_features", "bool_features"], outputCol="features"
    )

    PIPELINE = Pipeline(
        stages=[ASSEMBLER_NUM, SCALER, ASSEMBLER_BOOL, FINAL_ASSEMBLER, MODEL]
    )
...
def evaluate_model(pipeline: PipelineModel, test_data: DataFrame) -> dict:
    ...
    AUC_ROC = BinaryClassificationEvaluator(
        labelCol="TEP", rawPredictionCol="rawPrediction", metricName="areaUnderROC"
    )
    ACCURACY = MulticlassClassificationEvaluator(
        labelCol="TEP", predictionCol="prediction", metricName="accuracy"
    )
    F1 = MulticlassClassificationEvaluator(
        labelCol="TEP", predictionCol="prediction", metricName="f1"
    )
    RECALL = MulticlassClassificationEvaluator(
        labelCol="TEP", predictionCol="prediction", metricName="recallByLabel"
    )
    PRECISION = MulticlassClassificationEvaluator(
        labelCol="TEP", predictionCol="prediction", metricName="precisionByLabel"
    )
